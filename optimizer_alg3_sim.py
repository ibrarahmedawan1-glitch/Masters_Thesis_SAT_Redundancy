...
def solve_circuit(circuit_path, output_path):
    ...
    try:
        ...
        return orig_gates, orig_gates, 0, 0, timings

    if A == 0: 
        shutil.copy(process_path, output_path)
        timings["Total"] = time.time() - t_start_total
        return orig_gates, orig_gates, orig_gates, 0, timings
    timings["Parse"] = time.time() - t0

    t1 = time.time()
    if L == 0:
        h_sa0, h_sa1 = bit_parallel_filter(I, O, A, inputs, outputs, gates_raw)
        if A > 0:
            dropped_pct = ((2*A - (len(h_sa0) + len(h_sa1))) / (2*A)) * 100
            print(f"(Sim Dropped {dropped_pct:.1f}%)", end=" ")
    else:
        h_sa0, h_sa1 = list(range(A)), list(range(A)) 
    timings["Filter"] = time.time() - t1
    ...
    try:
        ...
        unique_removed = len(set([idx for idx, val in redundant_indices]))
        timings["Total"] = time.time() - t_start_total
        
        return orig_gates, orig_gates, count_reachable_gates(output_path), unique_removed, timings
