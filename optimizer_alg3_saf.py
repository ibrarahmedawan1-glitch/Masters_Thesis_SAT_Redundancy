...
def solve_circuit(circuit_path, output_path):
    ...
    try:
        M, I, L, O, A, inputs, latches, outputs, gates_raw, symbols = parse_aag_strict(circuit_path)
        orig_aig = aiger.load(circuit_path)
    ...
    build_universal_machine(circuit_path, univ_aag)
    redundant_indices = []
    ...
    try:
        ...
        with Solver(name='g4', bootstrap_with=cnf.clauses) as s:
            ...
            for i in range(A):
                # Check SA0
                if not s.solve(assumptions=base[:i] + [f0_lits[i]] + base[i+1:] + [miter_lit]):
                    redundant_indices.append((i, 0))
                # Check SA1
